<?php
    ...
    if (!isset($_POST['user_id']) || !isset($_POST['pet_name']) || !isset($_POST['pet_type']) || !isset($_POST['pet_gender']) || !isset($_POST['pet_health']) || !isset($_POST['category']) || !isset($_POST['pet_age']) || !isset($_POST['description']) || !isset($_POST['lat']) || !isset($_POST['lng']) || !isset($_POST['image'])) {
	...
	$sqlinsertpet = "INSERT INTO `tbl_pets`(`user_id`, `pet_name`, `pet_type`, `pet_age`, `pet_gender`, `pet_health`, `category`, `description`,`image_paths`,`lat`, `lng`) 
	VALUES ('$userid','$petName','$petType', '$petAge', '$petGender', '$petHealth', '$category','$description','','$lat','$lng')";
	try{
		if ($conn->query($sqlinsertpet) === TRUE){
            ...
            for($x = 0; $x < $countImage; $x++){
			    ...
			    $filename = "../uploads/pet/pets_".$last_id."_".($x + 1).".png";
